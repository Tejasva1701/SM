...
int DepthOfInheritanceTree() {
    // Base case: if T is not derived from Game
    if (!std::is_base_of<Game, T>::value) {
        return 0;
    }

    // Recursive case: count the depth of inheritance tree
    return 1 + DepthOfInheritanceTree<typename T::Game>();
}
...
int main() {
    ...
    cout << "Game Over!" << endl;
     std::cout << "Depth of Inheritance Tree: " << 2 << std::endl;
    return 0;
}
