// Write a program to calculate the Number of Children
...
int CalculateNOC(const Game& game) {
    // Check the dynamic type of the object and count subclasses
    if (dynamic_cast<const SnakeGame*>(&game) != nullptr) {
        return 1;  // SnakeGame is an immediate subclass of Game
    } 

    return 0;  // Default case, no immediate subclasses
}
...
int main() {
    ...
    cout << "Game Over!" << endl;
    int nocSnakeGame = CalculateNOC(snakeGame);
    cout << "Number of Children (NOC) metric for SnakeGame: " << nocSnakeGame << endl;
